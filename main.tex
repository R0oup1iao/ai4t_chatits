...
\end{itemize}
These outputs demonstrate SemCast's capacity to bridge the gap between prediction and action, transforming numerical data into operationally relevant intelligence. To systematically quantify the quality of the generated semantic reports and real-time recommendations, we conducted a bounded expert evaluation. A panel of three domain experts (senior traffic engineers, each with over 5 years of operational experience) blind-reviewed a randomly sampled subset of 50 generated reports. Output was scored on a 5-point Likert scale across four dimensions: factual consistency, actionable utility, logical coherence, and language clarity. \textcolor{blue}{The overall mean score across all dimensions was 3.87/5 (median: 4.00). To validate that these scores reflect a genuine consensus among raters rather than averaged diverging opinions, we computed Cronbach's alpha as an internal consistency measure across all three experts for each sample. The result yielded $\alpha = 0.86$, indicating substantial inter-rater agreement across the evaluation criteria \cite{cronbach_coefficient_1951}. Furthermore, a direct comparative analysis isolating the Retrieval-Augmented Generation module revealed that supplying historical precedent cases dropped the incidence rate of hallucinatory or physically unviable recommendations from 23\% to a manageable 6\%, verifying the operational reliability of the generated insights.}
...
\textcolor{blue}{Furthermore, the choice of $K$ presents a tunable trade-off between latency, computational overhead, and predictive accuracy. Reducing $K$ (e.g., to 10) proportionally decreases the computation time of Stage 1, further lowering end-to-end latency for large-scale deployments with strict real-time constraints; however, this reduction narrows the coverage of the probabilistic distribution, potentially missing rare or extreme anomaly trajectories. Conversely, while increasing $K$ (e.g., to 50) expands the hypothesis space, our empirical observations indicate that it does not yield further accuracy improvements. Instead, an excessively large candidate set introduces low-probability, long-tail noisy trajectories that can overcomplicate and confound the LLM's contextual reasoning process, while substantially increasing inference latency and token costs. Therefore, $K=20$ emerges as an optimal balance, providing sufficient trajectory diversity to capture anomalous patterns without overwhelming the LLM evaluator or sacrificing real-time efficiency. In practice, while operators can tune $K$ based on specific hardware constraints, $K=20$ serves as a highly robust default for operational deployments.}
...
\textcolor{blue}{Regarding cross-city transferability, SemCast's deployment in a new network with distinct topologies or traffic patterns would present specific challenges. Because the foundation model (Stage 1) is pre-trained on Beijing-specific data to capture local spatial-temporal dynamics, simply transferring it to another city without adaptation may lead to suboptimal performance due to distribution shifts. Efficient fine-tuning of the foundation model using a small amount of data from the target city would likely be necessary to adapt the spatial embeddings and statistical priors, rather than training entirely from scratch. The LLM-based components (Stages 2 and 3) are more inherently transferable; however, their effectiveness depends on updating the retrieval database with local operational precedents and potentially adjusting prompts to reflect regional traffic management norms. We plan to explore parameter-efficient fine-tuning strategies to facilitate low-cost cross-domain adaptation in future work.}

\section*{Acknowledgments}

The research is funded by the National Natural Science Foundation of China (No. T2588101) and Beijing Natural Science Foundation (No. JQ24051).
