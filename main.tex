...
\subsection{Stage 3: Retrieval-Augmented Decision Support}
The objective of Stage 3 is to translate the selected mathematical forecast $\hat{\mathbf{X}}^*$ into actionable operational insights. To ensure the recommendations are professionally grounded and adhere to standard operating procedures (SOPs), we employ a Retrieval-Augmented Generation (RAG) mechanism.

\subsubsection{Historical Knowledge Retrieval}
We construct a historical knowledge base $\mathcal{K} = \{(c_m, a_m)\}_{m=1}^M$, consisting of $M$ pairs of past traffic contexts $c_m$ (incident types, traffic states) and their corresponding expert management actions $a_m$ (e.g., signal timing adjustments, VMS activation).
To retrieve relevant few-shot examples for the current situation, we utilize a dense retriever. The current context $\mathbf{s}$ and forecast summary $\hat{\mathbf{X}}^*$ are concatenated into a query $q$. We compute the cosine similarity between the embedding of the query $\phi(q)$ and the stored context embeddings $\phi(c_m)$:
\begin{equation}
    \mathcal{S}_{rel} = \{ (c_m, a_m) \mid \cos(\phi(q), \phi(c_m)) > \tau \}
\end{equation}
where $\phi(\cdot)$ is a pre-trained sentence encoder and $\tau$ is a similarity threshold. The top-$N$ most similar pairs form the reference set $\mathcal{S}_{rel}$, providing the LLM with implicit ``operational priors.''

\subsubsection{Actionable Report Generation}
The final generation step synthesizes the scientific prediction with operational pragmatism. The LLM acts as a conditional generator $G$, producing a response $\mathbf{Y}$ composed of a situation report $\mathbf{Y}_{rep}$ and a recommendation list $\mathbf{Y}_{rec}$:
\begin{equation}
    \mathbf{Y} = G(\hat{\mathbf{X}}^*, \mathcal{T}_{ctx}, \mathcal{S}_{rel})
\end{equation}
The prompt structure enforces a dual-output format:
\begin{itemize}
    \item \textbf{Situation Report ($\mathbf{Y}_{rep}$):} A concise summary for decision-makers, highlighting the selected forecast's critical features (e.g., ``Expect 15-minute delays on Corridor B starting at 08:30 due to accident clearance'').
    \item \textbf{Operational Recommendations ($\mathbf{Y}_{rec}$):} Specific, actionable steps derived from the retrieved examples $\mathcal{S}_{rel}$ but adapted to the current $\hat{\mathbf{X}}^*$ (e.g., ``Implement Signal Plan 4 at Intersection X,'' ``Deploy diversion signage at Exit Y'').
\end{itemize}
This RAG-based approach mitigates generic or hallucinated advice, ensuring that the system's outputs are aligned with historical best practices.
...
We assessed the core contribution of Chat-ITS—integrating contextual information via LLM—using two distinct datasets: scheduled highway construction projects and unplanned traffic incidents. We compared the final Chat-ITS forecasts against a \textbf{Zero-shot} baseline (foundation model only) and a \textbf{Few-shot/Fine-tuned} baseline (foundation model fine-tuned on anomaly data).

As shown in Table \ref{table:2}, Chat-ITS demonstrated superior prediction accuracy compared to both baselines across all horizons. Compared to the Zero-shot forecast, Chat-ITS consistently reduced prediction errors (e.g., lower MAE). Crucially, Chat-ITS also outperformed the Fine-tuned strategy. This suggests that the LLM's ability to reason from explicit, real-time contextual descriptions (like construction schedules) is a more potent adaptation mechanism than relying solely on learning from limited historical patterns of disruptions.

Figure \ref{fig:2} provides illustrative examples. In a construction scenario (Figure \ref{fig:2}a), the foundation model underestimated the disruption severity. However, Chat-ITS, processing the text "one lane closed for 4 hours," inferred the non-linear impact on capacity and correctly predicted a sharp drop in speed and sustained low volume, aligning with ground truth. In an incident scenario (Figure \ref{fig:2}b), the foundation model assumed a quick recovery. In contrast, the LLM reasoned that the "full closure" and "diversion to bypass" would cause a severe, prolonged speed drop, yielding a forecast that closely tracked the observed traffic collapse. These results highlight how embedding domain-specific reasoning bridges the gap between statistical forecasting and operational reality.

\begin{figure}
\centering
\includegraphics[width=0.9\textwidth]{figure2_anomaly_forecasts}
\caption{\textbf{Chat-ITS enhances forecasting accuracy during anomalous events.} Time-series comparison of predicted traffic states versus ground truth. \textbf{(a)}, Highway construction scenario. \textbf{(b)}, Traffic incident scenario. The green dashed line (Chat-ITS) tracks the ground truth (black solid) more accurately than the foundation model baselines (red/blue dotted) by incorporating event context.}
\label{fig:2}
\end{figure}
