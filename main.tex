...
\subsection{Stage 1: Probabilistic Spatio-Temporal Foundation Model}
\subsubsection{Model Architecture \& Pre-training}
We adopt a T5 encoder-decoder Transformer architecture \cite{raffel2020exploring}, following the Chronos \cite{ansari_chronos_2024} paradigm for time-series forecasting. The key innovation is treating time-series forecasting as a language modeling task via tokenization.

\textbf{Input Representation \& Tokenization:} For each spatial node $i$, its univariate feature series (e.g., speed) $\mathbf{x}^i_{1:T}$ is processed independently in a channel-independent manner. The tokenization pipeline is:
\begin{enumerate}
\item \textbf{Temporal Patching:} The series is divided into overlapping patches of length $P$ with stride $S$ to capture local temporal patterns and reduce sequence length. Let $\mathbf{P}^i_j \in \mathbb{R}^P$ be the $j$-th patch.
\item \textbf{Scaling:} Each patch is normalized using mean scaling to stabilize training:
\begin{equation}
\mathbf{P}^{i,\text{scaled}}_j = \frac{\mathbf{P}^i_j}{\mu(\mathbf{P}^i_j) + \epsilon},
\end{equation}
where $\mu(\cdot)$ is the mean and $\epsilon$ a small constant.
\item \textbf{Quantization:} Scaled values are discretized into a vocabulary of $V$ bins via a learned scalar quantizer. Each value is assigned a token ID $z \in {1, ..., V}$. The sequence of token IDs for all patches forms the input "sentence" for the Transformer.
\end{enumerate}

\textbf{Encoder-Decoder Processing:} The encoder processes the tokenized historical sequence. The decoder autoregressively generates tokens representing the future patch sequence. The model is trained using a standard cross-entropy loss over the token vocabulary, predicting the next token in the patch sequence.

\textbf{Multi-Task Pre-training:} We pre-trained the model on a large corpus of historical traffic data from Beijing (see Section 4.1). To enhance its generalizability, we incorporated a secondary reconstruction task (masked patch prediction) alongside the primary forecasting task. This encourages the model to learn robust representations of traffic dynamics.

\subsubsection{Probabilistic Forecast Generation}
At inference, to generate the $K$ candidate trajectories for Stage 2:
\begin{enumerate}
\item \textbf{Autoregressive Sampling:} Instead of taking the argmax token at each step, we sample from the model's output probability distribution $p(z_t | z_{<t})$ using nucleus sampling (top-p) with $p=0.9$. This is repeated $K$ times to produce $K$ distinct token sequences ${\mathbf{Z}^{(k)}}{k=1}^K$.
\item \textbf{De-tokenization \& Aggregation:} Each token sequence is mapped back to continuous values via de-quantization and un-scaling using the original patch statistics. The patches are then aggregated (averaging overlapping regions) to reconstruct the full-length future series $\hat{\mathbf{x}}^{(k), i}{T+1:T+n}$ for each node $i$.
\item \textbf{Spatial Aggregation:} The univariate forecasts for all nodes are stacked to form the full spatio-temporal candidate $\hat{\mathbf{X}}^{(k)}_{T+1:T+n}$.
\end{enumerate}
...
\subsection{Stage 2: LLM for Contextual Adjustment}
\subsubsection{Contextual Prompt Construction}
...
\subsubsection{LLM Reasoning \& Selection}
We employ the Qwen2.5-14B-Instruct model \cite{qwen}. The model performs chain-of-thought reasoning:

\textbf{Context Interpretation:} Summarizes the events/incidents and their expected attributes (location, severity, duration, impacted roads).

\textbf{Impact Deduction:} Infers the qualitative impact on traffic (e.g., "The accident on Highway X will cause severe congestion southbound, spilling over to parallel route Y").

\textbf{Candidate Evaluation:} Compares each candidate's summarized traffic states against the deduced impact.

\textbf{Selection:} Outputs the index $k^*$ of the most plausible candidate and a brief reasoning text.
...
\subsection{Stage 3: Explainable Reporting \& Recommendation Generation}
\subsubsection{Prompt Design for Operational Output}
...
\subsubsection{Integration of Historical Management Patterns}
