...
\affil[3]{AutoNavi Software Co., Ltd. (AMAP), Alibaba Group, Beijing 100102, China}
...
Despite their expressive power, the necessity of complex graph convolutions has recently been questioned. Empirical studies indicate that simplified spatial modeling strategies can achieve comparable performance with substantially reduced computational overhead. SimST demonstrates that lightweight spatial aggregation approximates the effectiveness of GCN-based methods \cite{liu_we_2023}. Similarly, MLP-based architectures such as ST-MLP \cite{wang_st-mlp_2023} and STID \cite{shao_spatial-temporal_2022} show that concise spatio-temporal identity mappings can outperform complex GNNs by reducing overfitting. Furthermore, classical machine learning approaches remain highly viable for specific estimation tasks; for instance, Yuan et al. demonstrated that a Random Forest model, when enriched with sparse GPS, weather, and signal control data, effectively estimates bicycle delays at intersections without the need for deep architectures \cite{yuan_machine_2025}.
...
The application of Large Language Models (LLMs) in transportation research introduces a paradigm shift from purely numerical modeling toward semantic reasoning, multi-modal integration, and agent-based decision making \cite{nie_exploring_2025, karim_large_2025}. Comprehensive surveys categorize these emerging roles into information processors, knowledge encoders, and decision facilitators, highlighting the potential of LLMs to bridge the gap between heterogeneous data sources and actionable traffic insights \cite{nie_exploring_2025}. However, a central challenge lies in aligning continuous time series data with the discrete token-based representations of LLMs. Time-LLM and LLM4TS address this issue through reprogramming and fine-tuning strategies that encode numerical sequences as language tokens \cite{jin_time-llm_2023, chang_llm4ts_2023}.

In urban computing scenarios, UrbanGPT integrates spatio-temporal dependency encoders with instruction tuning to improve generalization under data scarcity \cite{li_urbangpt_2024}, while ST-LLM reformulates spatio-temporal observations as token sequences to capture global network dependencies \cite{liu_spatial-temporal_2024}. Beyond forecasting, LLMs have been explored for explanation, simulation, and decision support. TF-LLM and ChatTraffic generate natural language interpretations of traffic conditions and congestion causes, enhancing model interpretability \cite{guo_explainable_2024, zhang_chattraffic_2024}. In the domain of safety, CrashSage leverages LLMs to transform tabular crash data into textual narratives, enabling interpretable severity inference and uncovering complex risk factor interactions \cite{zhen_crashsage_2025}. Furthermore, CityGPT extends these capabilities by constructing a city-scale world model with LLM-agents \cite{feng_citygpt_2024}, while Liu et al. propose a framework where LLM-agents act as behaviorally rich proxies for human travelers to enhance microsimulation and travel demand modeling \cite{liu_toward_2025}.
...
\end{equation}
At each step $t$, the next token $z_t^{(k)}$ for the $k$-th hypothesis is sampled from the re-normalized distribution over $\mathcal{V}^{(p)}$. The stochasticity of the generated trajectories originates exclusively from this sampling procedure: each of the $K$ decoding passes draws tokens independently from the truncated categorical distribution using distinct random seeds, and the autoregressive nature of generation propagates this randomness across all subsequent time steps. Consequently, even with a fixed input, different sampling runs produce diverse trajectory hypotheses that collectively approximate the model's learned predictive distribution.
...
\end{equation}
where $\text{Description}(\cdot)$ maps numerical gradients to linguistic descriptors. Specifically, we define a set of threshold-based mapping rules for the average speed gradient $\bar{g}^{(k)} = \frac{1}{n-1}\sum_{t}(v^{(k)}_{t+1} - v^{(k)}_{t})$ computed across the prediction horizon: $\bar{g} > 5$ km/h per step $\to$ ``rapidly increasing''; $2 < \bar{g} \leq 5$ $\to$ ``gradually increasing''; $-2 \leq \bar{g} \leq 2$ $\to$ ``stable''; $-5 \leq \bar{g} < -2$ $\to$ ``gradually decreasing''; $\bar{g} < -5$ $\to$ ``rapidly decreasing.'' For example, a candidate with speed range $[25, 55]$ km/h and $\bar{g} = -6.2$ would be serialized as: ``Candidate 3: Trend $\in$ [25, 55] km/h, Dynamics: rapidly decreasing.'' This deterministic mapping ensures reproducibility and allows the LLM to comprehend the traffic dynamics implied by each hypothesis without processing raw tensors.

\subsubsection{Chain-of-Thought Reasoning and Selection}
We leverage the reasoning capabilities of the Qwen2.5-14B-Instruct model \cite{qwen} to model the causal relationship between the context $\mathbf{s}$ and traffic states. Critically, the LLM in this stage is restricted to a \textit{selection} operation over the pre-generated candidate set $\mathcal{H}$; it does not generate new numerical values or modify existing trajectories. This design ensures that all forecast outputs remain within the realistic traffic distributions learned by the foundation model, while the LLM contributes its semantic reasoning strength. The inference process is structured as a conditional probability maximization problem via Chain-of-Thought (CoT) prompting.
...
\end{enumerate}
The final output is the specific trajectory $\hat{\mathbf{X}}^* = \hat{\mathbf{X}}^{(k^*)}$, which represents the event-conditioned forecast. We emphasize that Equation~(10) defines a pure selection mechanism: $k^* \in \{1, \dots, K\}$ is an index into the candidate set, and the output $\hat{\mathbf{X}}^*$ is identical to one of the pre-generated trajectories without any numerical modification.

\subsection{Stage 3: LLM-Powered Reporting and Decision Support}
...
\end{equation}
where $\phi(\cdot)$ denotes a pre-trained sentence encoder and $\tau$ is a similarity threshold. The top-$N$ retrieved cases constitute the reference set $\mathcal{S}_{rel}$, which provides contextual constraints for the subsequent generation stage. It is important to note that the retrieved cases serve exclusively as \textit{in-context references} appended to the LLM prompt; they do not impose hard constraints or force the model to replicate historical actions. The LLM synthesizes information from the retrieved cases alongside the current forecast and context, enabling it to draw upon proven management patterns while adapting recommendations to the specific characteristics of the present scenario. This retrieval-augmented generation (RAG) approach also reduces the risk of hallucination by anchoring the LLM's outputs in documented operational precedents.
...
\end{itemize}
Table~S1 and Table~S2 in the Supplementary Materials provide representative examples of these two data types. In summary, the structured construction dataset contains 10,247 events with an average duration of 6.3 hours, spanning 892 distinct road segments across Beijing. Event types include full closure (18\%), partial closure (52\%), and shoulder work (30\%). The unstructured anomaly report dataset comprises 8,631 incident records, covering accidents (41\%), vehicle breakdowns (23\%), road hazards (19\%), and other incidents (17\%), with an average report length of 45 characters.
...
For implementation, we utilized established framework TSLib \cite{wu_timesnet_2023,wang2024tssurvey} or the official public code repositories associated with each baseline model. To guarantee a fair and direct comparison, all baseline models were trained using the identical historical dataset that was employed for training the SemCast foundation model. The sole exception was Chronos, for which we leveraged the publicly available pre-trained weights, applying it directly as a zero-shot forecaster without fine-tuning on our specific dataset. We note that this introduces a fundamental difference in experimental conditions: SemCast and the other baselines benefit from domain-specific pre-training on the Beijing traffic data, whereas Chronos relies entirely on its general-purpose pre-trained representations. The observed performance gap between Chronos and domain-trained models should therefore be interpreted as reflecting the challenge of zero-shot transfer to a specialized traffic forecasting domain, rather than a deficiency of the Chronos architecture itself. We include Chronos to illustrate the current limitations of general foundation models when applied to domain-specific traffic prediction without adaptation.
...
\textbf{Foundation Model Training Details}: The SemCast foundation model was trained jointly on the Beijing spatio-temporal grid data and the venue-centric traffic data described above, without incorporating external datasets. The model was trained for 50 epochs using the AdamW optimizer with an initial learning rate of $5 \times 10^{-4}$, a cosine annealing schedule with warm-up over the first 5 epochs, and a batch size of 256. The vocabulary size was set to $V = 4096$ bins, the patch length to $P = 12$ (corresponding to 1 hour), and the stride to $S = 6$ (30 minutes). The number of candidate trajectories was set to $K = 20$ with a nucleus sampling threshold of $p = 0.9$. Training was conducted on 4$\times$ NVIDIA A100 (80GB) GPUs over approximately 36 hours. For the LLM components in Stages 2 and 3, we employed the Qwen2.5-14B-Instruct model deployed via vLLM for efficient batch inference, without any fine-tuning.
...
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{fig2_semcast_scenarios}
...
\end{itemize}
These outputs demonstrate SemCast's capacity to bridge the gap between prediction and action, transforming numerical data into operationally relevant intelligence. While comprehensive automated evaluation of LLM-generated text remains an open challenge, we assessed report quality through two complementary approaches. First, a panel of three domain experts (traffic engineers with over 5 years of operational experience) independently evaluated 50 randomly sampled reports on four dimensions: factual consistency with the forecast data, actionability of recommendations, reasoning coherence, and language clarity, each on a 5-point Likert scale. The average scores were 4.3, 4.1, 4.2, and 4.5, respectively, indicating that the generated reports were considered reliable and practically useful. Second, we observed that the retrieval-augmented generation approach in Stage 3 substantially reduced instances of generic or hallucinated recommendations compared to a vanilla LLM baseline without case retrieval (reduction from 23\% to 6\% of outputs containing non-actionable suggestions). We acknowledge that more rigorous, large-scale evaluation protocols—including user studies in operational settings—remain an important direction for future work.
...
\begin{enumerate}
    ...
    \item \textbf{Context Quality:} While performance dropped when input context was intentionally degraded (vague/incorrect), SemCast still outperformed context-unaware baselines, indicating a degree of robustness (Figure \ref{fig:ablation_studies}c). Specifically, the \textit{vague} context condition was generated by stripping detailed attributes from event descriptions, retaining only the event category (e.g., replacing ``Two-vehicle collision on Ring Road 3 eastbound near Shuangjing Exit, blocking the rightmost lane'' with ``Traffic incident on Ring Road 3''). The \textit{incorrect} context condition was constructed by substituting contradictory event information (e.g., providing a ``road clear, normal flow'' description during an actual accident event). These degradation strategies test the model's sensitivity to context specificity and its resilience against misinformation, respectively.
\end{enumerate}

\begin{figure}
  \centering
  \includegraphics[width=0.9\textwidth]{fig4_semcast_ablation.pdf}
...
A key assumption of the framework is that the candidate trajectory set $\mathcal{H}$ generated by Stage 1 covers the event-induced traffic dynamics sufficiently for the LLM to select an appropriate forecast. This assumption is supported by two design choices: (1) nucleus sampling with $K=20$ trajectories and $p=0.9$ generates diverse hypotheses spanning a broad range of the learned predictive distribution; and (2) the foundation model is pre-trained on venue-centric data that includes non-standard traffic patterns (demand surges, rapid dispersal), thereby expanding the model's capacity to generate trajectories reflecting event-like dynamics even without explicit event conditioning. Our experimental results in Figure~\ref{fig:2} confirm that appropriate candidates are available across diverse anomaly types. Nevertheless, we acknowledge that for extreme or entirely novel event types whose traffic signatures differ fundamentally from any training data, the candidate set may fail to include sufficiently representative trajectories. Addressing this limitation—for instance, through guided sampling conditioned on event features or candidate augmentation strategies—is an important direction for future work.

Regarding computational feasibility, we provide a breakdown of the end-to-end inference latency measured on a single NVIDIA A100 GPU. Stage 1 (foundation model inference for $K=20$ trajectories) requires approximately 0.8 seconds per prediction window. Stage 2 (LLM-based trajectory selection via Qwen2.5-14B-Instruct, deployed with vLLM) requires approximately 2.1 seconds per query, including prompt construction and decoding. Stage 3 (report generation) requires approximately 3.5 seconds. The total pipeline latency of approximately 6.4 seconds is well within the 5-minute forecasting interval used in our system, confirming real-time feasibility. We note that further latency reductions are achievable through model quantization (e.g., INT4/INT8), smaller LLMs (e.g., 7B parameter variants), or parallel processing of Stages 2 and 3 across multiple sensors.
